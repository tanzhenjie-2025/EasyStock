...
def export_excel(request):
    """导出出车登记数据为 Excel（每栏25行，左右数据区间隔一列，信息列间隔一列，无边框）"""
    if request.method != 'POST':
